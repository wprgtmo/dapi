...
def get_federations(request: Request, profile_id: str, page: int=1, per_page: int=6, search: str="",  db: Session = Depends(get_db)):
    return get_all(request=request, page=page, per_page=per_page, profile_id=profile_id, criteria_value=search, db=db)

@federation_route.get("/federation/all/{profile_id}", response_model=Dict, summary="Obtain a list of Federations.")
def get_federations_all(request: Request, profile_id: str, db: Session = Depends(get_db)):
...
@federation_route.get("/federation/city/{profile_id}", response_model=ResultObject, summary="Get list of cities of Federation")
def get_city(request:Request, profile_id: str, db: Session = Depends(get_db)):
    return get_city_at_federation(request, profile_id=profile_id, db=db)
