...
@club_route.get("/club/all/{profile_id}", response_model=ResultObject, summary="Obtain a list of Clubs by profile_id")
def get_clubs_by_profile_id(request: Request, profile_id:str, db: Session = Depends(get_db)):
    return get_all_by_profile_id(request=request, profile_id=profile_id, db=db)
