...
from domino.services.paquete import get_all, get_all_data, new, get_one_by_id, delete, update
...
@paquete_route.get("/paquete", response_model=ResultObject, summary="Obtener lista de Paquetes con Paginado")
def get_paquetes(
    request: Request,
    page: int = 1, 
    per_page: int = 6, 
...
@paquete_route.get("/paquete/nomencladores", response_model=ResultObject, summary="Obtener lista de Paquetes sin paginado")
...
def get_paquete_by_id(id: int, db: Session = Depends(get_db)):
    return get_one_by_id(paquete_id=id, db=db)
...
def delete_paquete(request:Request, id: int, db: Session = Depends(get_db)):
    return delete(request=request, paquete_id=str(id), db=db)
