...
@post_route.get("/{post_id}/{file_name}", response_class=FileResponse, summary="Mostrar imagen de un post")
def getPostFile(post_id: str, file_name: str):
    return FileResponse(getcwd() + "/public/post/" + post_id + "/" + file_name)

@post_route.get("/{post_id}/{video_name}", summary="Mostrar un video de un Post")
