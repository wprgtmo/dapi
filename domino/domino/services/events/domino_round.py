...
def get_lst_player_shorted(tourney_id: str, round_id: str, str_order, db:Session):
    
    str_list_player = "Select puse.player_id, puse.profile_id, rsca.elo, rsca.elo_variable, rsca.acumulated_games_played, " +\
    ...
    str_list_player += str_order
    return db.execute(str_list_player).fetchall()
...
def configure_next_rounds(db_round, db:Session):
    ...
    str_order = get_str_to_order(db_round)
    
    lst_player_to_order = get_lst_player_shorted(db_round.tourney.id, db_round.id, str_order, db=db)
