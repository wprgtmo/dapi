...
def calculate_score_obtained(acumulated_games_played: int, points_difference: int): 
    return round(acumulated_games_played + (points_difference / 200), 4)
    
def calculate_score_expected(elo_win_pair: float, elo_lost_pair: float):  
    return round(1 / 1 + 10 ** ((elo_lost_pair - elo_win_pair) / 800), 4)
 
def calculate_increasing_constant(constant_increase_elo: float, number_games_played: int):  
    return round(constant_increase_elo + (300 / (300 + number_games_played)), 4)
    
def calculate_new_elo(increasing_constant: float, number_games_played: int, score_expected: float, score_obtained: float):  
    return 0 if number_games_played == 0 else round(increasing_constant * (score_obtained - score_expected), 4)
